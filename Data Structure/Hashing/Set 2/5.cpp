...
public:
    ...
    void reHashing(){
        vector<PhoneEntry*> old_table=table;
        table.clear();
        table_size*=table_size;
        table.resize(table_size);
        for(auto  & x : old_table){
          if(x and x!=deleted)  put(*x);
          delete x;
        }
        old_table.clear();
    }
	void put(PhoneEntry phone) {
		int org_idx= phone.hash() % table_size ;
        int step=1,idx=org_idx;
        do{
            ...
            idx = ( org_idx%table_size + (step*step)% table_size) % table_size ;
            step++;
        }while(idx!=org_idx);
        reHashing();
        put(phone);
	...
	bool get(PhoneEntry &phone) {
		...
		for (int step = 1 ; step <= sqrt(table_size); ++step) {
			if(!table[idx])
				break;
