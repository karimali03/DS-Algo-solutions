...
public:
	...
	void reverse_k_chain(int k){
		if(k <= 1) return;
		k = min( k , len);
		for(Node* cur = head,*prv = nullptr ; cur ; ){
			Node* st = cur; int cnt = 1;
			while(cur->next && cnt < k){
				cur = cur->next;
				cnt++;
			}
			auto lr = reverse_range(st,cur);
			if(prv) prv->next = lr[0];
			prv = lr[1];  cur = prv->next;
		}
	}
